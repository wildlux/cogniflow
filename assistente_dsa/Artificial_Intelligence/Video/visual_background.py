...
# MediaPipe imports (API legacy mp.solutions: rimossa nelle versioni recenti,
# quindi AttributeError va gestito come l'assenza del pacchetto)
try:
    ...
    logging.info("MediaPipe legacy (mp.solutions) non disponibile")

# MediaPipe Tasks API (versioni recenti): rilevamento mani con landmark
try:
    import mediapipe as mp  # type: ignore
    ...
    logging.warning("MediaPipe non disponibile - rilevamento mani via OpenCV")

# Modelli MediaPipe Tasks (scaricati in Video/models/)
MP_HAND_MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "models", "hand_landmarker.task"
)
MP_FACE_MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "models", "face_landmarker.task"
)

# Connessioni standard tra i 21 landmark della mano (per il disegno)
MP_HAND_CONNECTIONS = (
    (0, 1), (1, 2), (2, 3), (3, 4),
    (0, 5), (5, 6), (6, 7), (7, 8),
...
class VideoThread(QThread):
    ...
    def __init__(self, main_window=None):
        ...
        # MediaPipe Tasks (inizializzati pigramente al primo uso)
        self._hand_landmarker = None
        self._hand_landmarker_failed = False
        self._mp_last_ts = -1
        ...
        try:
            # cv2.data esiste solo nei pacchetti pip; l'OpenCV di sistema (Debian)
            # installa i cascade in /usr/share/opencv4/haarcascades
            cascades_dir = getattr(getattr(cv2, "data", None), "haarcascades", None)
            if cascades_dir is None:
                for candidate in (
                    "/usr/share/opencv4/haarcascades/",
                    "/usr/local/share/opencv4/haarcascades/",
                ):
                    if os.path.isdir(candidate):
                        cascades_dir = candidate
                        break
            if cascades_dir is None:
                raise FileNotFoundError("directory dei cascade Haar non trovata")
    ...
    def _get_face_landmarker(self):
        """Restituisce il FaceLandmarker MediaPipe, creandolo al primo uso."""
        if self._face_landmarker is not None or self._face_landmarker_failed:
            return self._face_landmarker
        if not MEDIAPIPE_TASKS_AVAILABLE or not os.path.exists(MP_FACE_MODEL_PATH):
            self._face_landmarker_failed = True
            return None
        try:
            options = mp_vision.FaceLandmarkerOptions(
                base_options=MPBaseOptions(model_asset_path=MP_FACE_MODEL_PATH),
                running_mode=mp_vision.RunningMode.VIDEO,
                num_faces=2,  # supporto per due persone
            ...
            logging.info("MediaPipe FaceLandmarker inizializzato (Tasks API)")
        except Exception as e:
            logging.warning(f"FaceLandmarker non inizializzabile: {e}")
            self._face_landmarker_failed = True
        return self._face_landmarker

    @staticmethod
    def _expression_from_blendshapes(blendshapes):
        """Traduce i blendshape MediaPipe in un'espressione semplice."""
    ...
    def _detect_faces_tasks(self, frame, landmarker):
        ...
        try:
            ...
            if timestamp_ms <= self._mp_face_last_ts:
                timestamp_ms = self._mp_face_last_ts + 1
            self._mp_face_last_ts = timestamp_ms
            result = landmarker.detect_for_video(mp_image, timestamp_ms)
        except Exception as e:
            logging.warning(f"Errore FaceLandmarker: {e}")
            return frame
        ...
        # Memorizza centro e area dei visi per assegnare le mani alle persone
        # (P1 = viso più grande, cioè persona più vicina alla camera)
        faces_info = []
        for idx, lms in enumerate(result.face_landmarks):
            ...
            faces_info.append(((x0 + x1) // 2, (y0 + y1) // 2, (x1 - x0) * (y1 - y0)))
        faces_info.sort(key=lambda f: f[2], reverse=True)
        ...
        for idx, lms in enumerate(result.face_landmarks):
            ...
            # Traccia leggera dei landmark (uno ogni 6 per non appesantire)
            for px, py in zip(xs[::6], ys[::6]):
                cv2.circle(frame, (px, py), 1, (255, 200, 120), -1)

            # Numero persona: P1 = viso più grande (più vicino alla camera)
            face_center = ((x0 + x1) // 2, (y0 + y1) // 2)
            person = 1
            for rank, (fx, fy, _area) in enumerate(faces_info, start=1):
                if abs(fx - face_center[0]) < 5 and abs(fy - face_center[1]) < 5:
                    person = rank
                    break
    ...
    def detect_faces(self, frame):
        """Rileva i visi: MediaPipe Tasks se disponibile, altrimenti OpenCV."""
        landmarker = self._get_face_landmarker()
        if landmarker is not None:
            return self._detect_faces_tasks(frame, landmarker)
    ...
    def detect_hand_gestures(self, frame):
        ...
        # Con MediaPipe Tasks attivo posizione e gesti sono già gestiti
        # in _detect_hands_tasks: evita doppioni e overlay contrastanti
    ...
    def _get_hand_landmarker(self):
        """Restituisce il HandLandmarker MediaPipe, creandolo al primo uso."""
        if self._hand_landmarker is not None or self._hand_landmarker_failed:
            return self._hand_landmarker
        if not MEDIAPIPE_TASKS_AVAILABLE or not os.path.exists(MP_HAND_MODEL_PATH):
            self._hand_landmarker_failed = True
            return None
        try:
            options = mp_vision.HandLandmarkerOptions(
                base_options=MPBaseOptions(model_asset_path=MP_HAND_MODEL_PATH),
                running_mode=mp_vision.RunningMode.VIDEO,
                num_hands=4,  # due persone: destra e sinistra di ciascuna
            ...
            logging.info("MediaPipe HandLandmarker inizializzato (Tasks API)")
        except Exception as e:
            logging.warning(f"HandLandmarker non inizializzabile: {e}")
            self._hand_landmarker_failed = True
        return self._hand_landmarker

    def _detect_hands_tasks(self, frame, landmarker):
        """Rilevamento mano con MediaPipe Tasks: posizione precisa + gesto.

        Emette hand_position_signal (centro palmo) e gesture_detected_signal
        ("Open Hand"/"Closed Hand"); nella zona ambigua non emette nulla per
        evitare cambi di stato spuri.
        """
        import time as _time
        ...
        try:
            ...
            mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
            timestamp_ms = int(_time.monotonic() * 1000)
            if timestamp_ms <= self._mp_last_ts:
                timestamp_ms = self._mp_last_ts + 1  # deve crescere sempre
            self._mp_last_ts = timestamp_ms
            result = landmarker.detect_for_video(mp_image, timestamp_ms)
        except Exception as e:
            logging.warning(f"Errore HandLandmarker: {e}")
            return frame
        ...
        for lms, handed in zip(result.hand_landmarks, result.handedness):
            # Il frame è specchiato: l'etichetta del modello va invertita
            # per corrispondere alla mano reale dell'utente
            model_label = handed[0].category_name if handed else "Right"
            user_side = "right" if model_label == "Left" else "left"

            if user_side == "right" and not self.right_hand_tracking_enabled:
                continue
            if user_side == "left" and not self.left_hand_tracking_enabled:
                continue
        ...
        # Assegna ogni mano a una persona usando i visi rilevati:
        # P1 = viso più grande (operatore vicino), P2 = l'altro
        faces = list(getattr(self, "_last_faces", []))[:2]
        for hd in hands:
            if len(faces) >= 2:
                wrist_x = hd["pts"][0][0]
                d1 = abs(wrist_x - faces[0][0])
                d2 = abs(wrist_x - faces[1][0])
        ...
        primary = (
            min(
                hands,
                key=lambda hd: (hd["person"] != 1, hd["side"] != "right"),
            )
            if hands
            else None
        )
        ...
        for hd in hands:
            is_primary = hd is primary
            # Colori: verde = primaria, arancio = altra mano P1, viola = P2
            if is_primary:
                bone_color = (0, 255, 0)
            elif hd["person"] == 1:
                bone_color = (255, 180, 60)
            else:
                bone_color = (255, 0, 200)
            for a, b in MP_HAND_CONNECTIONS:
                cv2.line(frame, hd["pts"][a], hd["pts"][b], bone_color, 2)
            for p in hd["pts"]:
                cv2.circle(frame, p, 3, (0, 200, 255), -1)

            cx, cy = hd["center"]
            if hd["gesture"] == "Open Hand":
                color = (0, 255, 0)
            elif hd["gesture"] == "Closed Hand":
                color = (0, 0, 255)
            elif hd["gesture"] == "Two Fingers":
                color = (255, 128, 0)  # azzurro: scorrimento
            else:
                color = (0, 255, 255)
            cv2.circle(frame, (cx, cy), 12, color, 3)

            side_label = "DX" if hd["side"] == "right" else "SX"
            tag = f"P{hd['person']} {side_label}{' *' if is_primary else ''}"
            cv2.putText(
                frame,
                tag,
                (cx - 20, cy - 20),
        ...
        if primary is not None:
            ...
            if primary["gesture"]:
                ...
                labels = {
                    "Open Hand": ("MANO APERTA", (0, 255, 0)),
                    "Closed Hand": ("MANO CHIUSA", (0, 0, 255)),
                    "Two Fingers": ("DUE DITA - SCORRI", (255, 128, 0)),
                }
                label, color = labels[primary["gesture"]]
                cv2.putText(
                    frame, label, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2
                )
        return frame

    def detect_hands(self, frame):
        """Rileva le mani: MediaPipe Tasks se disponibile, altrimenti OpenCV."""
        if not self.hand_detection_enabled:
            return frame

        landmarker = self._get_hand_landmarker()
        if landmarker is not None:
            return self._detect_hands_tasks(frame, landmarker)
    ...
    def determine_hand_orientation_advanced(
        ...
        try:
            ...
            thumb_based_result = self.analyze_thumb_position_improved(
                contour, x, y, w, h, is_left_side
            )
