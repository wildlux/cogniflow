...
class CPUHandlers:
    ...
    def initialize_cpu_monitor(self):
        ...
        try:
            self.main_window.cpu_monitor = CPUMonitor(
                self.main_window.settings, self.main_window
            )
            ...
            self.main_window.cpu_monitor.temperature_warning.connect(
                self.on_temperature_warning
            )
            self.main_window.cpu_monitor.temperature_critical.connect(
                self.on_temperature_critical
            )
    ...
    def on_temperature_critical(self, message):
        ...
        QMessageBox.critical(
            self.main_window,
            "Temperatura Critica - Terminazione",
