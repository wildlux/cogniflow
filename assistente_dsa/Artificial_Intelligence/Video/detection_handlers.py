...
class DetectionHandlers:
    ...
    def initialize_detection_system(self):
        ...
        try:
            detection_system = self.main_window.settings.get(
                "detection_system", "opencv"
            )
            ...
            if detection_system == "opencv":
                ...
                try:
                    ...
                    self.main_window.face_detector = (
                        VideoThread()
                    )  # Usa la stessa classe per entrambi
                    logging.info("Sistema di rilevamento OpenCV inizializzato")
                except ImportError:
                    logging.warning(
                        "OpenCV detector non disponibile, rilevamento disabilitato"
                    )
            ...
            logging.error(
                "Errore nell'inizializzazione del sistema di rilevamento: {e}"
            )
