...
class VoiceHandlers:
    ...
    def handle_voice_button(self):
        ...
        if not vosk_model or vosk_model == "auto":
            ...
            with open(
                "Save/SETUP_TOOLS_&_Data/settings.json", "w", encoding="utf-8"
            ) as f:
        ...
        self.main_window.speech_rec_thread.recognized_text.connect(
            self.on_voice_recognized
        )
        self.main_window.speech_rec_thread.recognition_error.connect(
            self.on_voice_error
        )
        self.main_window.speech_rec_thread.finished.connect(
            lambda: self.main_window.btn_voice.setEnabled(True)
        )
