...
# Import del bridge MediaPipe (opzionale)
try:
    from Artificial_Intelligence.Video.mediapipe.client.mediapipe_bridge import (
        register_mediapipe_bridge,
    )

    MEDIAPIPE_BRIDGE_AVAILABLE = True
except ImportError:
    register_mediapipe_bridge = None
    MEDIAPIPE_BRIDGE_AVAILABLE = False
...
def main():
    ...
    if MEDIAPIPE_BRIDGE_AVAILABLE:
        register_mediapipe_bridge(engine)
