...
# Gestore download in background (opzionale)
try:
...
class SettingsDialog(QDialog):
    ...
    def setup_general_tab(self):
        ...
        # Gruppo voce: parola d'ordine per l'ascolto continuo
        voice_group = QGroupBox("Voce")
        voice_layout = QVBoxLayout(voice_group)
        wake_layout = QHBoxLayout()
        wake_layout.addWidget(QLabel("Parola d'ordine:"))
        self.wake_word_edit = QLineEdit()
        self.wake_word_edit.setPlaceholderText("scrivi")
        self.wake_word_edit.setToolTip(
            "Con l'ascolto continuo attivo (🎙️), pronuncia questa parola\n"
            "seguita dal testo per inserirlo nel campo pensierino.\n"
            "Esempio: 'scrivi comprare il latte' → inserisce 'comprare il latte'"
        )
    ...
    def setup_download_tab(self):
        """Tab per scaricare in background modelli e componenti opzionali."""
        widget = QWidget()
        layout = QVBoxLayout(widget)

        info = QLabel(
            "I download avvengono in background: puoi chiudere questa "
            "finestra e continuare a usare CogniFlow.\n"
            "Riapri questa scheda in qualsiasi momento per vedere il progresso."
        )
        ...
        if not DOWNLOAD_MANAGER_AVAILABLE:
            layout.addWidget(QLabel("⚠️ Gestore download non disponibile"))
        else:
            for item_id, item in CATALOG.items():
                ...
                action_btn.setMinimumWidth(110)
                action_btn.clicked.connect(
                    lambda _, iid=item_id: self._on_download_action(iid)
                )
            ...
            # Vista sul gestore condiviso: i segnali arrivano anche per
            # download avviati altrove (es. dal pulsante vocale)
            download_manager.progress.connect(self._on_download_progress)
            download_manager.status.connect(self._on_download_status)
            download_manager.finished.connect(self._on_download_finished)

        layout.addStretch()
        self.tab_widget.addTab(widget, "📥 Download")

    def _refresh_download_row(self, item_id):
        """Allinea la riga allo stato attuale del gestore download."""
        row = self._download_rows.get(item_id)
        if row is None or download_manager is None:
            return
        if download_manager.is_installed(item_id):
            row["bar"].setValue(100)
            row["button"].setText("✅ Installato")
            row["button"].setEnabled(False)
            row["status"].setText("Componente pronto all'uso")
        elif download_manager.is_downloading(item_id):
            row["button"].setText("⛔ Annulla")
            row["button"].setEnabled(True)
            row["status"].setText("Download in corso…")
        else:
            row["bar"].setValue(0)
            row["button"].setText("📥 Scarica")
            row["button"].setEnabled(True)
            row["status"].setText("Non installato")

    def _on_download_action(self, item_id):
        if download_manager is None:
            return
        if download_manager.is_downloading(item_id):
            download_manager.cancel(item_id)
        else:
            download_manager.start(item_id)
        self._refresh_download_row(item_id)

    def _on_download_progress(self, item_id, percent):
        row = self._download_rows.get(item_id)
        if row is not None and percent >= 0:
            row["bar"].setValue(percent)

    def _on_download_status(self, item_id, message):
        row = self._download_rows.get(item_id)
        if row is not None:
